...
- **(افزوده ۲۰۲۶-۰۸-۲۰، فاز حرفه‌ای)** URL امضادار (Signed URL) برای فایل صوتی روی S3 — الان `AWS_QUERYSTRING_AUTH=False` یعنی URL فایل عمومی و دائمیه؛ حتی بعد از `toggle_track_visibility` (مورد جدید این فاز) فایل خام قابل‌دانلود مستقیم می‌مونه. اولویت بالا برای فاز بعدی امنیتی.
- **(افزوده ۲۰۲۶-۰۸-۲۰، فاز حرفه‌ای)** آپلود دسته‌ای چندفایلی برای آلبوم — الان هر ترک باید جدا از `upload.html` آپلود بشه.
- **(افزوده ۲۰۲۶-۰۸-۲۰، فاز حرفه‌ای)** زمان‌بندی انتشار (Scheduled Publish) — نیاز به فیلد جدید روی `Track` + یک job زمان‌بندی‌شده (Celery beat)؛ فعلاً انتشار فقط دستی/فوری از طریق `submit_track`/`approve_track`.
