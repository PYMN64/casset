...
## [2026-08-22] پاس رفع‌باگ/UX سراسری (پلیر، پلی‌لیست، اعلان‌ها، پروفایل) — ✅ بسته شد

**نوع:** Bugfix + Feature + Refactor
**برنچ:** `feature/s12-analytics-personalization` (روی همان برنچ S12، قبل از merge به `master`)
**فایل‌های تغییرکرده:** `accounts/{forms,views,tests,tests_email_verification,tests_rate_limit}.py`,
`accounts/backends.py` (جدید), `config/settings/base.py`, `interactions/{views,urls,tests}.py`,
`playlists/{views,tests}.py`, `tracks/{views,tests}.py`, `scripts/qa/journey_qa.py`,
...
**تصمیم:** درخواست مستقیم PYMN — یک لیست ~۱۹ موردی از باگ‌های واقعی و درخواست‌های
UX که در استفاده‌ی روزمره از سایت کشف شده بود، خارج از شماره‌گذاری S10-S13 نقشه‌ی
فاز ۲ (بدون رزرو کردن شماره‌ی S). به دستور صریح کاربر بدون توقف بین مراحل اجرا شد؛
هر مورد جداگانه پیدا، ریشه‌یابی، رفع و با تست پوشش داده شد:

1. **ثبت‌نام یوزرنیم دوباره می‌پرسید.** `RegisterForm` فیلد `username` جنگو
   (شناسه‌ی داخلی مبهم، نه یوزرنیم عمومی) را از کاربر می‌گرفت — درست همان چیزی
   که `CreatorHandleForm` دوباره، این‌بار به‌عنوان یوزرنیم عمومی واقعی، در گیت
   انتشاردهنده می‌پرسد. الان `register_view` مثل مسیر پیامک/گوگل از
   `unique_username()` تولید می‌کند و فیلد را از فرم حذف کرد. **پیامد جانبی
   اجباری:** چون کاربر دیگر یوزرنیمش را نمی‌بیند، ورود با رمز باید با ایمیل هم
   کار کند — `accounts/backends.py::EmailOrUsernameBackend` (زیرکلاس
   `AllowAllUsersModelBackend` موجود) اضافه شد؛ `AUTHENTICATION_BACKENDS` عوض
   شد؛ لیبل فرم ورود به «نام کاربری یا ایمیل» تغییر کرد.
2. **پلیر قابل بستن نبود.** دکمه‌ی `#pbClose` جدید (pause + hide، صف حفظ می‌شود).
3. **دکمه‌های × کار نمی‌کردند.** `qClose`/`plClose`/`embedClose` با
   `e.target.id === "..."` چک می‌شدند؛ چون هر سه یک `<svg>` داخلی دارند، کلیک
   روی خود آیکون (سطح بصری غالب دکمه) هیچ‌وقت با این شرط match نمی‌شد. با
   `.closest()` جایگزین شد.
4. **Repeat عملاً کار می‌کرد ولی آیکونش برای همیشه از بین می‌رفت.**
   `updateQueueUI` با `repBtn.textContent = "🔁"` آیکون SVG را بازنویسی و پاک
   می‌کرد. الان یک badge کوچک CSS (`.iconbtn--repeat-one::after`) وضعیت
   «تکرار: یک ترک» را نشان می‌دهد؛ svg دست‌نخورده می‌ماند. `pbShuffle`/`pbRepeat`
   و نسخه‌ی `np*` معادلشان هم اکنون هر دو همزمان sync می‌شوند (قبلاً فقط نسخه‌ی
   نوار کوچک آپدیت می‌شد).
5. **Shuffle بررسی شد:** منطق (`fisherYates` + نگه‌داشتن ترک جاری در جایگاه اول)
   درست است؛ با کوئری مستقیم در کنسول تایید شد که ترتیب واقعاً تصادفی می‌شود و
   خاموش‌کردن دوباره ترتیب اصلی را برمی‌گرداند. باگی پیدا نشد.
6. **دکمه‌ی + پلیر کلاً wire نشده بود.** `#npPlaylist` (نمای تمام‌صفحه) هیچ
   click handler ای نداشت. الان `#pbPlaylist` (جدید، در نوار کوچک هم) و
   `#npPlaylist` هر دو مودال افزودن به پلی‌لیست را برای ترک در حال پخش باز می‌کنند.
7. **مودال افزودن به پلی‌لیست:** فرم ساخت سریع inline اضافه شد (بدون رفتن به
   کتابخانه)؛ `api_playlist_mine` یک `track_id` اختیاری گرفت که `has_track`
   برمی‌گرداند تا دکمه‌ی هر پلی‌لیست آیکون افزودن/حذف واقعی (نه یک لینک متنی
   کور) نشان دهد.
8. **نوار پخش شلوغ بود.** آیکون‌های `.playerbar__controls`/`.playerbar__tools`
   کوچک‌تر شدند (۳۶→۳۲px)؛ breakpoint میانی جدید (۱۰۸۰px) قبل از حذف کامل
   ابزارها در ۷۲۰px، ابزارهای کم‌اهمیت‌تر (سرعت/تایمر خواب) را اول کنار می‌گذارد.
9. **لایک همیشه «نپسندیده» رندر می‌شد.** کارت‌های `_tcard.html` (Discover/
   Trending) هیچ context ای برای «آیا این کاربر این ترک را لایک کرده» نداشتند.
   به‌جای thread کردن این پرچم در همه‌ی view هایی که این partial را رندر
   می‌کنند، یک endpoint سبک جدید `GET /api/v1/likes/status/` اضافه شد +
   `app.js::hydrateLikeButtons()` که بعد از لود صفحه یک فراخوانی batched
   می‌زند. آیکون هم واقعاً بین `i-heart`/`i-heart-filled` سوییچ می‌کند (قبلاً
   فقط رنگ پس‌زمینه عوض می‌شد، و آن هم با یک کلاس CSS اشتباه — `primary` نه
   `iconbtn--primary`). تعداد لایک زیر آیکون روی کارت‌ها هم اضافه شد.
10. **دکمه‌ی فالو بعد از کلیک متنش عوض نمی‌شد.** `tracks/views.py::track_detail`
    و `accounts/views.py::_public_profile_context` حالا `is_following` را
    محاسبه و پاس می‌دهند؛ `handleFollow` متن («دنبال کردن» ↔ «لغو دنبال کردن»)
    و کلاس رنگ را واقعاً toggle می‌کند.
11. **منوی «⋯» زیر کارت بعدی می‌رفت.** `.card` به‌خاطر `backdrop-filter` یک
    stacking context مستقل می‌سازد، پس هر `z-index` داخلی‌اش هیچ‌وقت نمی‌تواند
    روی یک `.card` بعدی که دیرتر در DOM می‌آید برنده شود — دقیقاً چیزی که در
    صفحه‌ی ترک، منوی «⋯» را زیر «درباره این اثر» می‌برد. راه‌حل عمومی در
    `casset-ui.js::initMenus`: پنل باز به `<body>` منتقل (portal) و
    `position:fixed` با مختصات محاسبه‌شده از دکمه می‌شود — یک fix، همه‌ی
    صفحاتی که از `.menu`/`.menu__panel` استفاده می‌کنند (اکانت، «⋯» ترک، «⋯»
    پروفایل، اعلان‌های جدید) درست می‌شوند.
12. **نوتیفیکشن فقط یک لینک به صفحه‌ی کامل بود.** زنگوله حالا یک دراپ‌داون واقعی
    (`.menu__panel--notif`) دارد که آخرین ۱۰ اعلان را نشان می‌دهد
    (`GET /api/v1/notifications/`، از قبل موجود بود ولی فقط `unread_count`ش
    استفاده می‌شد). Badge شمارش همچنان فقط با mark-as-read واقعی صفر می‌شود.
13. **Toast زیر مودال پنهان می‌شد.** `.toast` z-index=120 < `.overlay-panel`
    z-index=130 بود. به ۲۰۰ افزایش یافت (بالاتر از هر overlay دیگر).
14. **کارت‌های Discover/Trending بازطراحی شدند.** از کادر مربعی به طرح «نوار
    کاست» (نسبت landscape، دو دایره‌ی قرقره‌مانند بالا، لیبل روی گرادیان پایین
    تصویر) — بدون حذف هیچ دکمه/آیکونی. فقط `_tcard.html` + بخش مربوطه‌ی
    `casset-ui.css` را لمس کرد (کلاس‌ها جای دیگری استفاده نمی‌شدند).
15. **دکمه‌های بالا/پایین صف پخش با ▲▼ متنی بودند.** به آیکون SVG (`i-up`/
    `i-down`) تغییر کردند، هم‌راستا با الگوی از‌قبل‌موجود در صفحه‌ی پلی‌لیست.
16. **تنظیمات حساب خیلی پهن بود و URL خام مدیا نشان می‌داد.** `.settings-panel`
    (`max-width:640px`) عرض فیلدها را محدود کرد؛ `ProfileSettingsForm` ویجت
    avatar/cover را از `ClearableFileInput` (که «Currently: <a href=…>» —
    مسیر خام فایل — رندر می‌کند) به `FileInput` ساده تغییر داد؛ باکس پیش‌نمایش
    خودش یک `<label for=…>` شد (کلیک = انتخاب فایل) با یک badge دوربین.
17. **متن/دکمه‌های روی کاور پروفایل.** لینک‌های اجتماعی و دکمه‌های اکشن (دنبال
    کردن/رادیو/اشتراک‌گذاری/گزارش) از روی خود عکس کاور به یک ردیف زیر کاور
    منتقل شدند؛ فقط نام/هندل/بیو کنار آواتار (جایی که گرادیان پایین کاور
    خوانایی را تضمین می‌کند) روی تصویر می‌مانند.

**باز مانده:** یک فلش جهت‌دار «روبروی متن آثار، سمت چپ» روی صفحه‌ی پروفایل که
PYMN گزارش داد ولی با بررسی کامل هر `i-up`/`i-down`/`i-chevron-*` در سایت
(هم در HTML رندرشده، هم به‌صورت CSS pseudo-element) پیدا نشد — تنها نامزد
سراسری (caret منوی حساب در نوار بالا) با توضیح کاربر جور درنمی‌آمد. اسکرین‌شات
واقعی در این session ممکن نبود (Browser pane قابل‌نمایش نبود). PYMN عکس/توضیح
دقیق‌تر می‌فرستد.

**اثر:** **۷۰۸ تست سبز روی SQLite** (از ۶۹۴ baseline S12، +۱۴ تست این پاس)،
**۷۰۹ روی PostgreSQL واقعی محلی** (همان ۵ شکست پیش‌موجود و نامرتبط
`core.tests_settings_secrets` — Winsock محلی ویندوز، هم قبل هم بعد از این
تسک روی هر دو دیتابیس تکرارپذیر)، `ruff check .` تمیز، بدون migration drift.
`scripts/qa/journey_qa.py` هم برای مسیر ثبت‌نام جدید (register → verify-email
→ onboarding، چون آن اسکریپت هنوز فرض قبل-از-S10 «ثبت‌نام مستقیم ریدایرکت به
onboarding» را داشت — یک شکاف قدیمی، از این تسک نبود، ولی چون همین تسک دقیقاً
همان مسیر را لمس می‌کرد این‌جا هم رفع شد) به‌روزرسانی شد: **۶۴ از ۶۴ ادعا سبز**
روی داده‌ی seed شده‌ی زنده. تایید دستی end-to-end در مرورگر (Playwright-ماننده
از طریق ابزار داخلی) با DOM assertions مستقیم برای هر ۱۷ مورد بالا — بدون
console error در هیچ صفحه.

**وضعیت CLAUDE.md:** خارج از شماره‌گذاری `.casset/releases/v2.1.0-phase2-plan.md`
(S10-S13) — یک تسک ad-hoc جداگانه به دستور مستقیم PYMN. آن سند دست‌نخورده ماند؛
S13 (اتصال بانکی) هنوز آخرین آیتم رسمی فاز ۲ است.
...
**درخواست کاربر:** ممیزی کامل کد/گیت/مستندات، بعد پاکسازی کامل Notion و CLAUDE.md/.claude/Skillها
با ثبت لاگ خیلی دقیق فاز ۱ قبل از هر پاکسازی، و آماده‌سازی سند فاز ۲.

**اثر — ممیزی (انجام‌شده قبل از این entry، همان روز):**
آلودگی line-ending (CRLF) در ۲۳۵ فایل working tree پیدا و رفع شد (صفر تغییر محتوایی
واقعی، تأیید با `git diff -b`). برنچ‌های زائد پاک شدند (`feat/orange-noir-v2` که دقیقاً
برابر `master` بود، `stabilization/v1-baseline`، `auto/2026-08-20-verify-settings-cleanup`
محلی؛ `Codex`، `docs/part-2-engineering-foundation` (از قبل merge شده)، `release/v2` (چک‌پوینت
رهاشدهٔ دسامبر ۲۰۲۵)، `codex/upgrade-genre-to-v2` (رهاشده، جایگزینش `enable_book` از قبل
روی master است) از ریموت. اسکریپت آواتار جاافتاده (`generate_avatars.py`) کامیت شد. پوشش
تست از ۸۱٪ به **۹۲٪** به‌روزرسانی شد (۵۹۱ تست). جزئیات کامل: `.casset/state/audit-2026-08-21.md`.

**اثر — مستندسازی نهایی فاز ۱ و ورود به فاز ۲:**
- سند جدید `.casset/releases/phase-1-final-report.md` — مرجع رسمی و دقیق «فاز ۱ تمام شد»:
  مسیر Sprint-به-Sprint (S0 تا S8)، فهرست کامل هر ۴۴ باگ/بدهی فنی رفع‌شده با شاهد، و نسخه‌های
  منتشرشده (v1.0 → v1.2.0 → v2.0.0). هدف: جلوگیری از اینکه جزئیات فاز ۱ در کامیت‌های پراکنده
  گم شود.
- سند جدید `.casset/releases/v2.1.0-phase2-plan.md` (از سشن قبلی همین روز) — مرجع کار فعلی.
- `CLAUDE.md` بخش ۳: جدول ۴۴ موردی (که همه ✅ بودند) در یک `<details>` آرشیو شد و با پوینتر به
  `phase-1-final-report.md` جایگزین شد — دیگر جدول «زنده» نیست. بخش ۵ و ۶ به فاز ۲ اشاره می‌کنند.
- `.casset/state/current.md`, `.casset/roadmap.md`, `.casset/state/task-queue.md` با پوینتر به
  اسناد فاز ۲ به‌روزرسانی شدند.
- **Notion به‌طور کامل sync شد** (کارگاه Notion از ۷ اوت ۲۰۲۶ کاملاً کهنه بود — می‌گفت
  «Sprint 0 فعال است»): صفحات «۰۵ — Roadmap»، «۰۶ — Technical Debt»، «۰۸ — Current State»
  بازنویسی کامل شدند؛ صفحهٔ قدیمی «۰۹ — نقشهٔ ۹۰ روزه» با بنر «آرشیو تاریخی» علامت‌گذاری شد
  (حذف نشد، طبق درخواست صریح کاربر: اول لاگ/تأیید، بعد پاکسازی)؛ دو صفحهٔ جدید «۱۰ — گزارش
  نهایی فاز ۱» و «۱۱ — نقشهٔ فاز ۲» زیر Project Brain ساخته شدند؛ دیتابیس «Sprint & Tasks»
  به‌روزرسانی شد (۱۸ ردیف قدیمی که واقعاً تمام شده بودند از Backlog به Done تغییر کردند، ۱۱
  ردیف جدید فاز ۲ اضافه شدند).
- سه Skill پروژه (`casset-audit`, `casset-sync-docs`, `casset-task`) بازبینی و برای ساختار
  جدید (جدول آرشیوی به‌جای جدول زنده) به‌روزرسانی شدند.

**تصمیم صریح دربارهٔ حذف:** طبق دستور کاربر، هیچ سند تاریخی حذف نشد — همه یا آرشیو شدند
(`<details>` در CLAUDE.md، بنر «آرشیو» در Notion) یا به سند جدید منتقل شدند. فقط فایل‌های
واقعاً بی‌ارزش (یک فایل تست موقت `.casset/_write_test.tmp` ساختهٔ همین ایجنت) حذف پیشنهاد شد.

**وضعیت CLAUDE.md:** بخش ۳ به‌طور کامل بازساختاردهی شد (نه حذف محتوا، فقط جابه‌جایی/آرشیو).

**نکته برای session بعد:** خواندن اجباری از این به بعد: `phase-1-final-report.md` برای
تاریخچه، `v2.1.0-phase2-plan.md` برای کار فعلی. جدول بخش ۳ CLAUDE.md دیگر منبع حقیقت زنده
نیست — فقط آرشیو است.

---

## [2026-08-21] Orange Noir v2 — بازطراحی فرانت‌اند + ورود گوگل + قانون انتشاردهنده — ✅ بسته شد

**نوع:** Architecture + Bugfix + Refactor
**تگ:** `v2.0.0` · **مبنای قبلی:** `v1.2.0` (`f396b3c`)
**دامنه:** ۱۱۱ فایل · ۱۱٬۰۱۳+ / ۲٬۹۷۶− · ۳۸ فایل جدید
**سند کامل:** `.casset/releases/v2.0.0-mvp.md`

**تصمیم:**
صاحب پروژه یک بازطراحی حرفه‌ای فرانت‌اند («Orange Noir v2») همراه با سه فیچر
بک‌اندی خواست: ورود با گوگل، امکان انتشار برای هر کسی که یوزرنیم انتخاب کند
(با الزام شمارهٔ موبایل)، و پنل ورود حرفه‌ای‌تر. هدف صریح: «نسخهٔ نهایی بدون
باگ و خطر امنیتی، قابل ارائه برای ۱۰۰۰ کاربر».

طبق Constitution بازنویسی انجام **نشد**: کل کار روی همان Django templates و
همان `static/app.js` سوار شد و قرارداد `data-*` دست‌نخورده ماند.

**اثر — باگ‌های واقعی که پیدا و رفع شدند:**

1. **فونت Vazirmatn هرگز لود نمی‌شد.** CSP خود پروژه
   (`style-src 'self'`) استایل‌شیت Google Fonts را که `base.html` صدا می‌زد
   بلاک می‌کرد. کل سایت با فونت پیش‌فرض سیستم رندر می‌شد. فونت self-host شد و
   CSP به‌جای بازتر شدن سخت‌تر شد (`font-src 'self'`, `frame-ancestors`,
   `base-uri`, `form-action`, `object-src`).
2. **XSS ذخیره‌شده در JSON-LD.** `json.dumps` کاراکتر `<` را escape نمی‌کند؛
   عنوان `</script><img onerror=…>` از بلوک اسکریپت فرار می‌کرد. همان سه
   escape که `django.utils.html.json_script` می‌زند اعمال شد.
3. **کامنت چندخطی `{# #}` روی همهٔ صفحات به‌عنوان متن چاپ می‌شد** (کامنت جنگو
   تک‌خطی است). ۴ قالب، از جمله `base.html`. دو تست نگهبان اضافه شد.
4. **اسکرول افقی روی موبایل در همهٔ صفحات** (۴۲۴px در ویوپورت ۳۷۵px). سه علت:
   `min-width: auto` روی `.search`، `width: 25%` روی نوار پایینِ ۵ آیتمی، و
   `1fr` خام در گریدها (`minmax(auto, 1fr)` که به min-content حل می‌شود).
5. **تم روشن WCAG AA را رد می‌کرد.** سفید روی دکمهٔ اصلی ۲.۳۵:۱ بود. توکن
   «رنگ به‌عنوان متن» از «رنگ به‌عنوان fill» جدا شد؛ حالا کمترین ۵.۰۲:۱.
6. **مودال تایید هندلر AJAX را دور می‌زد** (`form.submit()` رویداد submit را
   شلیک نمی‌کند) و کاربر روی JSON خام می‌افتاد → `requestSubmit()`.
7. **درگ‌اند‌دراپ پلی‌لیست** به endpointی پست می‌کرد که فقط جابه‌جایی
   تک‌پله‌ای می‌فهمید → پذیرش `{"order": [...]}` با چک‌های مالکیت.
8. **opt-out اعلان از یک reverse-OneToOne کش‌شده خوانده می‌شد** و بی‌صدا
   نادیده گرفته می‌شد → کوئری مستقیم.
9. **فایل‌های استاتیک هش نشده + service worker با cache-first** یعنی دیپلوی
   به کاربر نمی‌رسید → `ManifestStaticFilesStorage` و SW فقط مسیر precache
   می‌کند. همین اجرا یک `sourceMappingURL` مردهٔ Chart.js را هم لو داد.
10. **رگرسیون خودساخته:** مذاکرهٔ محتوا فقط `X-Requested-With` را چک می‌کرد
    ولی `fetch` خود `app.js` آن را نمی‌فرستاد → هر دو سیگنال پذیرفته شد.

**اثر — فیچرهای جدید:**

- **ورود با گوگل** (`accounts/oauth.py`): جریان authorization-code استاندارد
  OIDC با PKCE(S256)، state گره‌خورده به session، و nonce. **allauth استفاده
  نشد** — ۴۰ پکیج provider، URL namespace، قالب و سه مدل اضافه برای یک
  provider روی رابطی که کاملاً سفارشی است. امضای ID token محلی چک نمی‌شود
  (توکن مستقیم از endpoint TLS گوگل می‌آید، طبق مستندات خودشان) ولی همهٔ
  claimها اعتبارسنجی می‌شوند. ایمیل تاییدنشده کاملاً رد می‌شود — پذیرفتنش یک
  مسیر تصاحب حساب بود. تطبیق: اول `google_sub` بعد ایمیل.
- **قانون انتشاردهنده**: `UserProfile.can_publish` = یوزرنیم عمومی + شمارهٔ
  تاییدشده. گیت روی **ارسال برای بررسی** است نه آپلود — پیش‌نویس خصوصی و
  بی‌ضرر است. فیچر جدید لازم: تایید شماره برای حساب واردشده
  (`/account/phone/`)، چون جریان OTP قبلی فقط خارج‌از‌حساب بود.
- **تنظیمات اعلان**: `NotificationPreference` با یک نقطهٔ اعمال در
  `services.py::_allowed`. نبودن ردیف = همه روشن. `track_approved` و
  `track_rejected` قابل ساکت‌کردن نیستند.
- **بازیابی رمز** با viewهای خود جنگو.
- **سئو**: sitemap، robots تولیدشده، JSON-LD به‌ازای نوع محتوا، عنوان یکتای
  زیر ۶۰ کاراکتر، canonical، og:image پیش‌فرض برند.
- **PWA**: ست آیکون، manifest با shortcuts، service worker با استراتژی
  به‌ازای نوع منبع (رسانه هرگز cache نمی‌شود)، صفحهٔ آفلاین.

**اثر — طراحی:**

- توکن‌های کامل Orange Noir v2 در `static/app.css`؛ تم روشن حالا یک تم واقعی
  و مستقل است (hack قبلی در `base.html` حذف شد).
- `static/css/casset-ui.css` (لایهٔ کامپوننت) و `static/css/cassette.css`
  (هویت پلیر: قرقره‌های چرخان با `animation-play-state`، برچسب کاغذی،
  اسکرابر رنگ‌نوار، `prefers-reduced-motion`).
- اسپرایت آیکون از ۲۴ به ۸۱ نماد؛ ایموجی‌های خام پروفایل حذف شدند.
- **`style=` در قالب‌ها از ۴۰۵ به ۱** (تنها مورد باقی‌مانده یک عرض واقعاً
  داینامیک است).

**تصمیم صریح: Vite/Alpine/htmx اضافه نشدند.**
سند طراحی پیشنهادشان داده بود. افزودن Vite یعنی وابستگی Node روی سرور
production و یک مرحلهٔ build در CI، برای سایتی با دو فایل استاتیک دستی — ریسک
عملیاتی بدون سود کاربری. چیزی که قرار بود بدهد از راه ارزان‌تر گرفته شد:
cache-busting از `ManifestStaticFilesStorage` خود جنگو، و سازمان‌دهی از تفکیک
لایه‌های CSS. قابل بازبینی است اگر بعداً تیم فرانت مستقل بیاید.

**تست:**
- ۵۰۲ → **۵۹۱** تست سبز (SQLite) · **۵۹۱** روی PostgreSQL ۱۶ واقعی
- `ruff check .` تمیز · `check --deploy` زیر تنظیمات prod تمیز
- `scripts/qa/journey_qa.py` — ۶۰ ادعای مسیر واقعی روی دیتابیس زنده
  (**دو باگ را همین پیدا کرد** که تست واحد نگرفته بود)
- خزش ۱۵۲ صفحه در ۴ نقش — صفر خطا
- کنتراست هر جفت توکن در دو تم — صفر شکست، کمترین ۵.۰۲:۱
- سرریز افقی: ۲۱ صفحه در ۳۷۵px و ۵ صفحه در ۴ عرض دسکتاپ — صفر

**وضعیت CLAUDE.md:** موارد #۳۵ تا #۴۴ اضافه و بسته شدند؛ بخش ۳، domain map و
نسخهٔ مبنا آپدیت شدند.

**نکته برای session بعد:**
`.casset/releases/v2.0.0-mvp.md` سند مرجع این فاز است. قبل از هر کار روی
رنگ‌ها، `static/app.css` بخش توکن را بخوان — «رنگ به‌عنوان fill» و «رنگ
به‌عنوان متن» دو توکن جداگانه‌اند و قاطی‌کردنشان کنتراست را می‌شکند.
قبل از هر تغییر روی گرید، `minmax(0, 1fr)` را حفظ کن.
